...
def load_model():
    ...
    if model_instance is None:
        ...
        try:
            ...
            print(f"[!] Erro ao carregar o modelo AI (Provavelmente incompatível com 0-index): {e}", flush=True)
            model_instance = None
...
def choose_setup(board: list[list[Cell]]) -> SetupResponse:
    ...
    if not candidates:
        candidates = [(r, c) for r in range(BOARD_SIZE) for c in range(BOARD_SIZE) if board[r][c].professor is None]
...
def choose_turn(board: list[list[Cell]], team_id: int) -> Optional[PlayerTurnResponse]:
    ...
    if not current_team_name: return None
...
def random_fallback(board: list[list[Cell]], team_id: int) -> Optional[PlayerTurnResponse]:
